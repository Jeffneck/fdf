...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/20 17:35:46 by hanglade          #+#    #+#             */
/*   Updated: 2023/11/28 15:13:57 by hanglade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	erase_printed(char *buffer_dest, char *buffer_src, char **line)
{
	int	i;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || !line || BUFFER_SIZE < 1)
		return (free(line), NULL);
	buffer = init_buffer(fd, fd_buffs);
	if (buffer == NULL)
		return (free(line), NULL);
	if (!erase_printed(buffer, &buffer[check_endline(buffer)], &line))
		return (free(line), NULL);
